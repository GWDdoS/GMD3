...
geode::Result<geode::Ref<GJLevelList>> ImportGmdList::intoList() {
...
geode::Result<> gmd::exportListAsGmd(GJLevelList* list, std::filesystem::path const& to, GmdListFileType type) {
    return ExportGmdList::from(list).setType(type).intoFile(to);
}

geode::Result<geode::Ref<GJLevelList>> gmd::importGmdAsList(std::filesystem::path const& from) {
